...
cluster_data_1 <- function(cvd, source, time_scale)
{
    ## Get the data
    ...
    del <- mclapply(del_orig,
                    function(x) filter_year(croston_smooth(x), time_scale),
                    mc.cores = 8)

    print("Preprocessing consumption data")
    con <- mclapply(con_orig,
                    function(x) filter_year(ksmooth_xts(x, kernel = "normal", bandwidth = 50), time_scale),
                    mc.cores = 8)
...
clus_tel_day <- cluster_data_1(cvd, source = "tel", time_scale = "day")
...
ggsave("./papers/paper1/del_clus_mean_vert_9_mv.eps", plot = pl, device = cairo_ps, width = 6.5, height = 6.5, units = "in", dpi = 1000)

plot_mv_centroids(clus)
ggsave("./papers/paper1/del_clus_mean_cent_9.eps", device = "eps", width = 6.5, height = 4.5, units = "in", dpi = 1000)
...
ggsave("./papers/paper1/mv_clus_mean_vert_9.eps", plot = pl, device = cairo_ps, width = 6.5, height = 6.5, units = "in", dpi = 1000)

plot_mv_centroids(mv_clus_mean_9)
ggsave("./papers/paper1/mv_clus_mean_cent_9.eps", device = "eps", width = 6.5, height = 4.5, units = "in", dpi = 1000)
